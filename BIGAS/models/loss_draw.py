...
class LossHistory(keras.callbacks.Callback):
    ...
    def loss_plot(self, loss_type):
        ...
        plt.figure()
        # acc
        plt.plot(iters, self.accuracy[loss_type], lw=1, color='r', label='train acc', marker='.', markevery=1,
                 mew=1)
        # loss
        # plt.plot(iters, self.losses[loss_type], lw=1.5, color='g', label='train loss', marker='.', markevery=2, mew=1.5)
        if loss_type == 'epoch':
            # val_acc
            plt.plot(iters, self.val_acc[loss_type], lw=1, color='b', label='val acc', marker='.', markevery=1,
                     mew=1)
            # val_loss
